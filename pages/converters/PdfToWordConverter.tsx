...
const PdfToWordConverter: React.FC<PdfToWordConverterProps> = ({ tool }) => {
  ...
  const handleConvert = () => {
    ...
    setDownloadUrl(null);
    setTimeout(() => {
      const mockContent = `This is a mock DOC file from ${file.name}.`;
      const blob = new Blob([mockContent], { type: 'application/msword' });
      ...
      <div className="w-full max-w-xl mx-auto flex flex-col items-center">
        {!downloadUrl ? (
          ...
          <div className="text-center">
            ...
            <p className="text-brand-text-secondary mt-2">Your Word document is ready to download.</p>
            <a href={downloadUrl} download={file?.name.replace(/\.pdf$/, '.doc') || 'converted.doc'} className="mt-6 inline-block bg-green-500 text-white px-8 py-3 rounded-md hover:bg-green-600 font-bold shadow-lg shadow-green-500/30 hover:shadow-xl hover:shadow-green-500/50 transition-all text-lg">
              Download .doc File
            </a>
        ...
        <p className="text-xs text-brand-text-secondary mt-8 text-center max-w-md">
            Note: This browser-based converter is best for simple, text-based PDFs. Complex layouts, fonts, and images may not be preserved accurately.
        </p>
      </div>
    </ToolPageLayout>
  );
