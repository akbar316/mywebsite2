...
const ImageToPdfConverter: React.FC<ImageToPdfConverterProps> = ({ tool }) => {
  ...
  const handleConvert = () => {
    ...
    setDownloadUrl(null);
    setTimeout(() => {
      const mockContent = `This is a mock PDF file generated from the image ${file.name}.`;
      ...
      <div className="w-full max-w-xl mx-auto flex flex-col items-center">
        {!downloadUrl ? (
          <>
            ...
            >
              <input type="file" id="file-input" className="hidden" accept="image/*" onChange={(e) => handleFileChange(e.target.files)} />
              <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" className="text-brand-text-secondary mb-4"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
              <p className="text-brand-text-primary">Drag & drop your image here</p>
              <p className="text-sm text-brand-text-secondary">or click to select a file (JPG, PNG, GIF, etc.)</p>
            </div>
            {error && <p className="mt-4 text-red-500">{error}</p>}
            {file && !isConverting && (
              <div className="mt-6 text-center">
                <p className="text-brand-text-secondary">Selected file:</p>
                <img src={URL.createObjectURL(file)} alt="Preview" className="max-w-xs max-h-40 my-2 mx-auto rounded-md shadow-md" />
          ...
          <div className="text-center">
            ...
            <a href={downloadUrl} download={file?.name.replace(/\.[^/.]+$/, "") + '.pdf' || 'converted.pdf'} className="mt-6 inline-block bg-green-500 text-white px-8 py-3 rounded-md hover:bg-green-600 font-bold shadow-lg shadow-green-500/30 hover:shadow-xl hover:shadow-green-500/50 transition-all text-lg">
              Download .pdf File
            </a>
