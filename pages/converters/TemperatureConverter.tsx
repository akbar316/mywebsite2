...
const TemperatureConverter: React.FC<TemperatureConverterProps> = ({ tool }) => {
  ...
  const result = useMemo(() => {
    ...
    let kelvin = 0;
    if (fromUnit === 'celsius') kelvin = numValue + 273.15;
    else if (fromUnit === 'fahrenheit') kelvin = (numValue - 32) * 5/9 + 273.15;
    else kelvin = numValue;

    let convertedValue = 0;
    if (toUnit === 'celsius') convertedValue = kelvin - 273.15;
    else if (toUnit === 'fahrenheit') convertedValue = (kelvin - 273.15) * 9/5 + 32;
    else convertedValue = kelvin;
  ...
  const unitOptions = units.map(u => <option key={u} value={u}>{u.charAt(0).toUpperCase() + u.slice(1)}</option>);
