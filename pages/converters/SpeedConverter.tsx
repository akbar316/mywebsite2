...
const SpeedConverter: React.FC<SpeedConverterProps> = ({ tool }) => {
  const [value, setValue] = useState<number | string>(1);
  const [fromUnit, setFromUnit] = useState<Unit>('km/h');
  const [toUnit, setToUnit] = useState<Unit>('miles/h');

  const result = useMemo(() => {
    const numValue = typeof value === 'string' ? parseFloat(value) : value;
    if (isNaN(numValue)) return '';
    const valueInMetersPerSec = numValue / units[fromUnit];
    const convertedValue = valueInMetersPerSec * units[toUnit];
  ...
  const unitOptions = Object.keys(units).map(u => <option key={u} value={u}>{u}</option>);
